...
column_index = []
for i in range(1, len(areas)+1):
    min_.append(compare['Cell-'+str(i)+'_Min'].min())
    dr_.append(((compare['Cell-'+str(i)+'_Min'].min()-compare['Cell-'+str(i)+'_Min'].iloc[0])/compare['Cell-'+str(i)+'_Min'].iloc[0])*100)
    column_index.append('Cell-'+str(i)+'_Min')
change = pd.DataFrame(list(zip(column_index, min_, dr_)), columns =['Cell','Min', 'DR']).set_index('Cell', drop=True)
